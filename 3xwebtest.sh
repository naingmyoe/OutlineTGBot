...
# 2. index.html ဖိုင်ကို ရေးသားထည့်သွင်းခြင်း (Updated with Public Key)
echo -e "${GREEN}Writing updated index.html with Public Key field...${NC}"
...
<script>
    ...
    function loadFormData() {
        ...
        document.getElementById('txtPub').value = config.realityKey || ''; // Load Key
    ...
    window.saveConfig = async () => {
        ...
        config.realityKey = document.getElementById('txtPub').value; // Save Key
